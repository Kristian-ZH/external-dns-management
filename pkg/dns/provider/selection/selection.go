...
func CalcZoneAndDomainSelection(spec v1alpha1.DNSProviderSpec, zones []LightDNSHostedZone) SelectionResult {
	...
	excludedSubdomains := collectForwardedSubdomains(this.DomainSel.Include, this.Zones)
...
func collectForwardedSubdomains(domains utils.StringSet, zones []LightDNSHostedZone) utils.StringSet {
	result := utils.StringSet{}
	for d := range domains {
		for _, z := range zones {
			if d == z.Domain() {
